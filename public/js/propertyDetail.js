...
socket.on('connect', function(){
    ...
    socket.emit('whoAmI', function(params){
      ...
      socket.emit('loadProperty', {propertyID: decodeURIComponent(window.location.href.substring(window.location.href.lastIndexOf('/')+1))}, function(propertyInfo){
...
loadDetails = function(propObj){
  ...
  console.log('Set Address: ', propObj.Address);
  newDetail.innerHTML = propObj.Address;
